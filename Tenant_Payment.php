...
// Retrieve email and propertyId from POST or GET request
$email = $_POST['email'] ?? $_GET['email'] ?? null;
$propertyId = $_POST['propertyid'] ?? $_GET['propertyid'] ?? null;

if ($email && $propertyId) {
    // Fetch booking details
    $stmt = $pdo->prepare("SELECT bookingId FROM bookings WHERE email = ? AND propertyId = ?");
    $stmt->execute([$email, $propertyId]);
    ...
    if ($booking) {
        ...
        if ($priceDetails) {
            ...
            echo 'No price details found for the given property ID.';
            exit;
        }
    } else {
        echo 'No booking found for the given email and property ID combination.';
        exit;
    }
} else {
    echo 'Email or Property ID not provided.';
    exit;
}
?>
<!DOCTYPE html>
