...
function update_profile($mysqli) {
    ...
    if (!empty($name) && !empty($email) && !empty($country_code) && !empty($phone_number) && !empty($gender) && !empty($nationality) && !empty($occupation) && !empty($usertype)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Profile updated successfully');window.location.href='Landlord_Profile.php'</script>";
        } else {
            $_SESSION['error'] = "Error in updating profile: " . $stmt->error;
            echo "<script>alert('Error in updating profile: " . $stmt->error . "');window.location.href='Landlord_Profile.php'</script>";
        }
    } else {
        $_SESSION['error'] = "Please fill in all required fields";
        echo "<script>alert('Please fill in all required fields');window.location.href='Landlord_Profile.php'</script>";
    }
...
<head>
    ...
    border-radius: 50%; /* 确保容器边框是圆形 */
    width: 104px; /* 宽度需要比图片稍大，包含边框宽度 */
    height: 104px; /* 高度需要比图片稍大，包含边框宽度 */
...
<section class="header">
    ...
    <form action="Landlord_Profile.php" method="POST" enctype="multipart/form-data">
