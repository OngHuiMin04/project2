...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        header("Location: Landlord_HomeDetails.php?propertyId=$property_id&success=Appointment%20successful");
        exit();
    } catch (Exception $e) {
        // Redirect to homedetails.php with error message
        header("Location: Landlord_HomeDetails.php?propertyId=$property_id&error=" . urlencode("Error: " . $e->getMessage()));
        exit();
    }
} else {
    // Redirect to homedetails.php if accessed directly without POST data
    header("Location: Landlord_HomeDetails.php");
    exit();
}
?>
